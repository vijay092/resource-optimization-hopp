...
def quick_lcoh_approx(
    ...
    wind_CapEx_USD = (wind_size_MW * 1000) * year_costs[
        "Wind CapEx [$/kW]"
    ]  # wind_capex_kW
    solar_CapEx_USD = (solar_size_MW * 1000) * year_costs[
        "Solar CapEx [$/kW]"
    ]  # solar_capex_kW
    # doube check below
    battery_CapEx_kW = (
    ...
    wind_OpEx_USD = (wind_size_MW * 1000) * year_costs[
        "Wind OpEx [$/kW-year]"
    ]  # *wind_opex_kWyr
    ...
    electrolyzer_refurbishment_cost_USD[refturb_period:plant_life:refturb_period] = (
    ...
    hydrogen_storage_CapEx_USD = (
        hydrogen_storage_CapEx_kg * hydrogen_storage_capacity_kg
    )
...
for cost_yr in cost_years:
    ...
    lcoh = quick_lcoh_approx(
        wind_size_mw,
        solar_size_mw,
        battery_size_mw,
        battery_hrs,
        target_annual_hydrogen_kg,
...
def calc_hydrogen_storage_size(hourly_h2_production):
    ...
    before_storage_CF = np.sum(hourly_h2_production) / (
        electrolyzer_size_MW * h2_bol[-1] * 8760
    )
    ...
    compressor_capex = np.interp(
        hydrogen_flow_capacity_kg_day,
        compressor_cost_data["Nameplate kg/d"].to_numpy(),
        compressor_cost_data["Capital Cost [$]"].to_numpy(),
    )
    compressor_FOM_frac = np.interp(
        hydrogen_flow_capacity_kg_day,
        compressor_cost_data["Nameplate kg/d"].to_numpy(),
        compressor_cost_data[
            "Fixed Operating Cost [fraction of OvernightCapCost/y]"
        ].to_numpy(),
    )
    compressor_FOM_USD_yr = compressor_FOM_frac * compressor_capex

    pipeline_capex_perkm = np.interp(
        hydrogen_flow_capacity_kg_day,
        pipeline_cost_data["Nameplate kg/d"].to_numpy(),
        pipeline_cost_data["Capital Cost [$/km]"].to_numpy(),
    )
    pipeline_capex = pipeline_capex_perkm * pipeline_length_km
    pipeline_FOM_frac = np.interp(
        hydrogen_flow_capacity_kg_day,
        pipeline_cost_data["Nameplate kg/d"].to_numpy(),
        pipeline_cost_data[
            "Fixed Operating Cost [fraction of OvernightCapCost/y]"
